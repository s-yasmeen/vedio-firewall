...
const state={history:[],installPrompt:null};
...
const fallbackReps={
  expression:['action_units','expression_embedding'],
  movement:['landmark_trajectories','motion_embedding'],
  rppg:['physiological_signal'],
  authentication:['cancelable_template'],
  visual_exam:['protected_video']
};
async function loadRepresentations(){
  const task=$('#task').value, select=$('#representation');
  let reps=fallbackReps[task]||[];
...
function requestPayload(){
  return {
    task:$('#task').value,representation:$('#representation').value,measured_at:isoNow(),max_evidence_age_seconds:300,
    privacy_upper_threshold:n('#privacyThreshold'),utility_lower_threshold:n('#utilityThreshold'),temporal_upper_threshold:n('#temporalThreshold'),latency_ms_threshold:n('#latencyThreshold'),
    operating_points:[{
      alpha:n('#alpha'),identity_risk:n('#identityRisk'),identity_risk_upper:n('#identityUpper'),task_utility:n('#taskUtility'),task_utility_lower:n('#utilityLower'),
      temporal_risk:n('#temporalRisk'),temporal_risk_upper:n('#temporalUpper'),latency_ms:n('#latency'),evaluator_id:$('#evaluatorId').value.trim()||'edge-evaluator-v1',sample_count:Math.max(1,Math.floor(n('#sampleCount')||1))
    }]
  };
}
...
function renderDecision(d){
  ...
  $('#selectedAlpha').textContent=d.selected_alpha??'—';$('#selectedRep').textContent=d.representation||'—';$('#evidenceHash').textContent=shortHash(d.evidence_sha256);$('#requestId').textContent=d.request_id||'—';
  $('#riskRingValue').textContent=release?'PASS':'BLOCK';
}
function addHistory(d){
  const att=d.attestation||{}; state.history.unshift({decision:d.decision||att.release_decision||'BLOCK',task:d.task||att.task||'—',representation:d.representation||att.representation||'—',request_id:d.request_id||att.request_id||'—',time:att.timestamp_utc||isoNow()});
  state.history=state.history.slice(0,12);renderHistory();
}
function renderHistory(){
  const root=$('#attestationList'); root.innerHTML='';
  if(!state.history.length){root.innerHTML='<article class="panel empty">No release evaluations in this session.</article>';return}
  state.history.forEach(a=>{const card=document.createElement('article');const rel=String(a.decision).toUpperCase()==='RELEASE';card.className='panel attestation-card '+(rel?'release':'block');card.innerHTML=`<span class="attestation-dot" aria-hidden="true"></span><div><strong>${rel?'RELEASE':'BLOCK'} · ${escapeHTML(a.task)}</strong><small>${escapeHTML(a.representation)} · ${escapeHTML(a.request_id)}</small></div><time>${new Date(a.time).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</time>`;root.appendChild(card)});
}
...
$('#releaseForm').addEventListener('submit',async e=>{
  ...
  msg.textContent='Evaluating measured evidence…';
  const key=$('#apiKey').value.trim();
  try{const d=await fetchJSON('/v2/release/evaluate',{method:'POST',headers:key?{Authorization:`Bearer ${key}`}:{},body:JSON.stringify(p)});renderDecision(d);addHistory(d);msg.textContent='Evaluation complete.'}
...
$('#loadBlocked').addEventListener('click',()=>{
  $('#identityRisk').value='0.88';$('#identityUpper').value='0.94';$('#taskUtility').value='0.82';$('#utilityLower').value='0.76';$('#temporalRisk').value='0.73';$('#temporalUpper').value='0.86';$('#latency').value='95';$('#formMessage').textContent='Loaded a high identity-leakage example. Run the gate to confirm BLOCK.';
});
