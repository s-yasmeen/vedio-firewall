...
- detected faces may be eye-aligned;
- a trusted prior box may be reused only for a very short detector dropout;
- there is no center/full-frame fallback because scene/clothing/camera cues can leak identity;
- invalid frames become neutral crops and are explicitly masked from temporal attention.

Detection/alignment/reuse masks are validation evidence only and must never be released as task output.
"""
...
class FacePreprocessor:
    def __init__(self, *, output_size: int = 112, margin: float = 0.20, max_reuse_frames: int = 2):
        if output_size < 32:
            raise ValueError("output_size too small")
        if not 0 <= margin <= 1:
            raise ValueError("margin must be in [0,1]")
        if int(max_reuse_frames) < 0 or int(max_reuse_frames) > 5:
            raise ValueError("max_reuse_frames must be in [0,5]")
        if not hasattr(cv2, "CascadeClassifier"):
            raise RuntimeError(
                "OpenCV build lacks CascadeClassifier; use the pinned supported OpenCV 4.x research dependency"
            )
    ...
    def _align_by_eyes(self, crop_rgb: np.ndarray) -> tuple[np.ndarray, bool]:
        ...
        eyes = self.eye_detector.detectMultiScale(
            gray, scaleFactor=1.1, minNeighbors=4, minSize=(10, 10)
        )
        if len(eyes) < 2:
        ...
        # Reject implausible rotations rather than warping a false eye pair.
        if abs(angle) > 25.0:
            return crop_rgb, False
    ...
    def crop_face(self, rgb: np.ndarray, *, previous_bbox=None) -> FaceCropResult:
        ...
        crop = cv2.resize(
            crop, (self.output_size, self.output_size), interpolation=cv2.INTER_AREA
        )
        return FaceCropResult(crop, detected, aligned, True, reused, bbox)

    def process_sequence(self, frames_rgb):
        crops, detections, alignments, valid, reused = [], [], [], [], []
        prev = None
        reuse_streak = 0
        for frame in frames_rgb:
            allow_prev = prev if reuse_streak < self.max_reuse_frames else None
            res = self.crop_face(frame, previous_bbox=allow_prev)
            ...
            if res.detected and res.bbox is not None:
                prev = res.bbox
                reuse_streak = 0
            elif res.reused_bbox:
                reuse_streak += 1
            else:
                prev = None
                reuse_streak = 0
